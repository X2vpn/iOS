...
class X2ForgetPassOtp: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func submitTapped(){
        ...
        if(APIManager.shared.checkInternetAvailable()){
            startLoader()
            APIManager.shared.callForgetVerificationAPI(username: email!, token: captcha, completion: { (result, message) in
                self.stopLoader()
                if(result == "Success"){
                    UserDefaults.standard.setValue(captcha, forKey: "forgetOtp")
    ...
    func gotoSetPassword(){
        DispatchQueue.main.async {
            ...
            let vc = storyboard.instantiateViewController(withIdentifier: "X2SetNewPass")
