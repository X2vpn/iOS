...
extension UIViewController {
    func showToast(message: String) {
        ...
        toastView.backgroundColor = UIColor(named: "D6CEFF")
        toastView.layer.cornerRadius = 10
        toastView.translatesAutoresizingMaskIntoConstraints = false

        // Create imageView
        let imageView = UIImageView()
        imageView.image = UIImage(named: "x2Logo")
        ...
        label.font = UIFont(name: "Inter-Medium", size: 14.0)
        label.text = message
        label.translatesAutoresizingMaskIntoConstraints = false
        toastView.addSubview(label)

        // Add toastView to the view hierarchy
        self.view.addSubview(toastView)

        // Activate constraints
        NSLayoutConstraint.activate([
            imageView.centerYAnchor.constraint(equalTo: toastView.centerYAnchor), // Center vertically in toastView
            ...
            // Set the maximum width of toastView to 300
            toastView.widthAnchor.constraint(lessThanOrEqualToConstant: 300)
        ])

        // Ensure toastView has a minimum 100 units from the bottom
        let bottomConstraint = toastView.bottomAnchor.constraint(lessThanOrEqualTo: self.view.bottomAnchor, constant: -100)
        bottomConstraint.priority = .required
        bottomConstraint.isActive = true

        // Center toastView horizontally
        toastView.centerXAnchor.constraint(equalTo: self.view.centerXAnchor).isActive = true

        // Animate and remove toastView after a delay
