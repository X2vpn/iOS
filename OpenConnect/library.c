...
int openconnect_setup_tun_device(struct openconnect_info *vpninfo,
                                 const char *vpnc_script, const char *ifname, void *tb)
{
	intptr_t tun_fd;
	char *legacy_ifname;
    ...
    const struct oc_ip_info *info_ip;
    openconnect_get_ip_info(vpninfo, &info_ip, NULL, NULL);
    // 5.
    int sb = test_externc(info_ip, tb);
