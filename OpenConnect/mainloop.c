...
static int setup_tun_device(struct openconnect_info *vpninfo, void * tb)
{
	int ret;
...
#endif
	ret = openconnect_setup_tun_device(vpninfo, vpninfo->vpnc_script, vpninfo->ifname, tb);
	if (ret) {
...
int openconnect_mainloop(struct openconnect_info *vpninfo,
                         ...
                         int reconnect_interval, void * tb
                         )
{
	int ret = 0;
	...
	while (!vpninfo->quit_reason) {
		...
		if (vpninfo->dtls_state > DTLS_DISABLED) {
			...
			if (!tun_is_up(vpninfo) && (vpninfo->dtls_state == DTLS_CONNECTED ||
				...
				ret = setup_tun_device(vpninfo, tb);
				if (ret) {
					break;
				}
			}
