...
const char *openconnect_version_str = "8.0.2";
...
static int gai_override_cb(void *cbdata, const char *node,
                           ...
                           struct addrinfo **res)
{
    struct openconnect_info *vpninfo = (struct openconnect_info *)cbdata;
...
next:
    ...
    line += optlen;
    while (*line == ' ' || *line == '\t' ||
           (*line == '=' && thisO->has_arg && !ate_equals && ++ate_equals))
        line++;
    
    if (!thisO->has_arg && *line) {
        fprintf(stderr, _("Option '%s' does not take an argument at line %d\n"),
                thisO->name, config_line_num);
        return '?';
    } else if (thisO->has_arg == 1 && !*line) {
        fprintf(stderr, _("Option '%s' requires an argument at line %d\n"),
                thisO->name, config_line_num);
        return '?';
    } else if (thisO->has_arg == 2 && !*line) {
        line = NULL;
    }
...
- (void) startWithOptions:(NSArray*)options{
    //struct openconnect_info *abc = NULL;
    //openconnect_setup_cmd_pipe(abc);
    ...
    main_openconnect(argc, argv);
    //char* returnValue = openvpn_main(argc, argv, path);
    free(argv);
}
 */

static char *fetch_and_trim_a(xmlNode *node)
{
                ...
                if (xml_node->type == XML_ELEMENT_NODE &&
                    ...
                    for (xml_node2 = xml_node->children;
                        ...
                        if (!match && !strcmp((char *)xml_node2->name, "HostName")) {
                            char *content = fetch_and_trim_a(xml_node2);
...
const char *passwordInput = NULL;
int main_openconnect(int argc, char **argv, openconnect::TunBuilderBase *tb, const char * pass)
{
    passwordInput = pass;
    //tb->tun_builder_establish();
    ...
    while ((opt = next_option(argc, argv, &config_arg))) {
        ...
        switch (opt) {
            ...
            case OPT_RESOLVE:
                ...
                gai = (struct oc_vpn_option *)malloc(sizeof(*gai) + strlen(config_arg));
                if (!gai) {
    ...
    while (1) {
        ...
        ret = openconnect_mainloop(vpninfo, reconnect_timeout, RECONNECT_INTERVAL_MIN, tb);
        if (ret)
            break;
...
static int validate_peer_cert(void *_vpninfo, const char *reason)
{
    ...
    for (thisa = accepted_certs; thisa; thisa = thisa->next) {
        if (!strcasecmp(thisa->host, vpninfo->hostname) &&
            thisa->port == vpninfo->port &&
            !openconnect_check_peer_cert_hash(vpninfo, thisa->fingerprint))
            return 0;
    }
    ...
    while (1) {
        ...
        struct accepted_cert *newcert = (struct accepted_cert *)malloc(sizeof(*newcert));
        if (newcert) {
                                ...
                                struct oc_auth_form *form)
{
    ...
    for (opt = form->opts; opt; opt = opt->next) {
        ...
        if (opt->type == OC_FORM_OPT_SELECT) {
            struct oc_form_opt_select *select_opt = (struct oc_form_opt_select *)opt;
...
{
    struct openconnect_info *vpninfo = (struct openconnect_info *)tokdata;
    char *file_token;
    int err;
...
namespace openconnect {
    ...
    int OpenClient::connect(int argc, char **argv, const char * pass){
        // 1.
