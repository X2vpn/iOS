...
class TunnelsManager {
    ...
    static func create(completionHandler: @escaping (Result<TunnelsManager, TunnelsManagerError>) -> Void) {
        ...
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            ...
            if (tunnelManagers.count ?? nil)! > 0 {
                tunnelManager = tunnelManagers.first
                ...
                if ((proto?.migrateConfigurationIfNeeded(called: tunnelManager?.localizedDescription ?? "X2VPN")) != nil) {
            ...
            for loadedTunnelProvider in loadedTunnelProviders {
                if let matchingTunnel = self.tunnels.first(where: { loadedTunnelProvider.isEquivalentTo($0) }) {
                    ...
                    if let proto = loadedTunnelProvider.protocolConfiguration as? NETunnelProviderProtocol {
                        if proto.migrateConfigurationIfNeeded(called: loadedTunnelProvider.localizedDescription ?? "X2VPN") {
    ...
    func add(tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption = .off, completionHandler: @escaping (Result<TunnelContainer, TunnelsManagerError>) -> Void) {
        ...
        tunnelProviderManager.localizedDescription = "X2VPN"

        // Check If KillSwitch Is On
        if(UserDefaults.standard.bool(forKey: "KillSwitchEnabled")){
            let onDemandRule = NEOnDemandRuleConnect()
            onDemandRule.interfaceTypeMatch = .any
            tunnelProviderManager.onDemandRules = [onDemandRule]
    ...
    func modify(tunnel: TunnelContainer, tunnelConfiguration: TunnelConfiguration,
        ...
        let oldName = tunnelProviderManager.localizedDescription ?? "X2VPN"
...
class TunnelContainer: NSObject {
    ...
    init(tunnel: NETunnelProviderManager) {
        name = tunnel.localizedDescription ?? "X2VPN"
...
extension NETunnelProviderManager {
    ...
    func setTunnelConfiguration(_ tunnelConfiguration: TunnelConfiguration) {
        ...
        localizedDescription = "X2VPN"
        objc_setAssociatedObject(self, &NETunnelProviderManager.cachedConfigKey, tunnelConfiguration, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    }

    func isEquivalentTo(_ tunnel: TunnelContainer) -> Bool {
        return localizedDescription == "X2VPN" && tunnelConfiguration == tunnel.tunnelConfiguration
    }
}
